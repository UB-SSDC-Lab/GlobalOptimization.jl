...
@testset showtiming=true "Full DE Optimization" begin
    ...
    res1 = GlobalOptimization.optimize!(
        GlobalOptimization.SerialDE(prob; num_candidates=20, max_iterations=100)
    )
    Random.seed!(seed)
    res2 = GlobalOptimization.optimize!(
        GlobalOptimization.ThreadedDE(prob; num_candidates=20, max_iterations=100)
    )
    Random.seed!(seed)
    res3 = GlobalOptimization.optimize!(
        GlobalOptimization.PolyesterDE(
            prob; num_candidates=20, max_iterations=100, max_time=1.0
        ),
    )
