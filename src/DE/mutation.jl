...
mutable struct MutationParameters{
    ...
    function MutationParameters(F1, F2, F3, F4; sel = SimpleSelector())
        S = typeof(sel)
        new{NoAdaptation, Unified, S, Nothing}(F1, F2, F3, F4, sel, nothing)
    end
    ...
    function MutationParameters(
        ...
        new{RandomAdaptation, MS, typeof(sel), typeof(dist)}(
...
struct SelfMutationParameters{
    ...
    function SelfMutationParameters(
        ...
        new{RandomAdaptation, MS, typeof(sel), typeof(dist)}(Fs, sel, dist)
    end
end
