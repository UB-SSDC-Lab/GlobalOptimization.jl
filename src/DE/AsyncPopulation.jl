...
"""
    DEBaseAsyncPopulation{T <: AbstractFloat} <: AbstractPopulation{T}

The base representation of some population for the async DE algorithms.
"""
struct DEBaseAsyncPopulation{T<:AbstractFloat} <: AbstractPopulation{T}
    # Data
    candidates::Vector{Vector{T}}
    candidates_fitness::Vector{T}

    # Locks
    c_locks::Vector{ReentrantLock}
    f_locks::Vector{ReentrantLock}
    ...
    function DEBasePopulation{T}(num_candidates::Integer, num_dims::Integer) where {T}
        return new{T}(
            ...
            [ReentrantLock() for _ in 1:num_candidates],
            [ReentrantLock() for _ in 1:num_candidates],
        )
    end
end
...
"""
    AsyncDEPopulation{T <: AbstractFloat} <: AbstractPopulation{T}

The full population representation for the DE algorithms, including both the candidates
and the mutants.
"""
struct AsyncDEPopulation{T<:AbstractFloat}
